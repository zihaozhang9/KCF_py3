...
@jit
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels):#计算hog特征
    ...
    for j in range(1, height-1):
        for i in range(1, width-1):
            ...
            for ch in range(1, numChannels):#如果有多个通道，取出最大的方向梯度的x,y
                tx = dx[j, i, ch]
                ty = dy[j, i, ch]
            ...
            for kk in range(0, NUM_SECTOR): #分为9个扇形共10个方向向量，找出梯度方向最大的方向
...
def getFeatureMaps(image, k, mapp):
	...
	mapp['map'] = np.zeros(int(mapp['sizeX']*mapp['sizeY']*mapp['numFeatures']), np.float32)#创建特征矩阵

	dx = cv2.filter2D(np.float32(image), -1, kernel)   # np.float32(...) is necessary #用卷积核水平方向滤波
	dy = cv2.filter2D(np.float32(image), -1, kernel.T) #垂直方向滤波，得到的应该是一张图片

	arg_vector = np.arange(NUM_SECTOR+1).astype(np.float32) * np.pi / NUM_SECTOR #角度向量
	...
	### 500x speedup
	#import pdb;pdb.set_trace()
	mapp['map'] = func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, int(stringSize)) #with @jit
	### ~0.001s
...
def normalizeAndTruncate(mapp, alfa):
	...
	### 50x speedup
	#import pdb;pdb.set_trace()
	idx = np.arange(0, int(sizeX*sizeY*mapp['numFeatures']), int(mapp['numFeatures'])).reshape((-1, 1)) + np.arange(p) #int(sizeX*sizeY)
