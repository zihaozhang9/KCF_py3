...
def complexMultiplication(a, b): #乘法
...
def rearrange(img):
	...
	img_[0:yh,0:xh] = img[yh:2*yh,xh:2*xh] #img[yh:img.shape[0],xh:img.shape[1]]
	img_[yh:2*yh,xh:2*xh]  = img[0:yh,0:xh] #img_[yh:img.shape[0],xh:img.shape[1]]  = img[0:yh,0:xh]
	
	img_[0:yh,xh:2*xh]  = img[yh:2*yh,0:xh] #img_[0:yh,xh:img.shape[1]]  = img[yh:img.shape[0],0:xh]
...
def limit(rect, limit):
	if(rect[0]+rect[2] > limit[0]+limit[2]):#如果宽超出图像尺寸
		rect[2] = limit[0]+limit[2]-rect[0] #限制一下w
	if(rect[1]+rect[3] > limit[1]+limit[3]):
		rect[3] = limit[1]+limit[3]-rect[1] #限制一下h
	if(rect[0] < limit[0]): #
...
def subwindow(img, window, borderType=cv2.BORDER_CONSTANT):
	cutWindow = [x for x in window] # x y w h #左上角xy 
	limit(cutWindow, [0,0,img.shape[1],img.shape[0]])   # modify cutWindow #限制下目标框位置
	assert(cutWindow[2]>0 and cutWindow[3]>0)
	border = getBorder(window, cutWindow) #获取超出框
	res = img[cutWindow[1]:cutWindow[1]+cutWindow[3], cutWindow[0]:cutWindow[0]+cutWindow[2]]
	#裁剪出目标图像
	if(border != [0,0,0,0]): #如果不为0
		res = cv2.copyMakeBorder(res, border[1], border[3], border[0], border[2], borderType) #则在原来的基础上增加边界条
	return res
...
class KCFTracker:
	...
	def subPixelPeak(self, left, center, right): #亚像素峰？
		divisor = 2*center - right - left   #float
		return (0 if abs(divisor)<1e-3 else 0.5*(right-left)/divisor) #相当于获取中心的偏移

	def createHanningMats(self): #创建汉宁窗
	...
	def gaussianCorrelation(self, x1, x2):
		if(self._hogfeatures):
			...
			for i in range(self.size_patch[2]): #通道数
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		cx = self._roi[0] + self._roi[2]/2  #float #中心点x
		cy = self._roi[1] + self._roi[3]/2  #float #中心点y

		if(inithann):
			padded_w = self._roi[2] * self.padding #目标宽#padding=2.5
			padded_h = self._roi[3] * self.padding #目标高

			if(self.template_size > 1): #template_size=96
				if(padded_w >= padded_h): #_scale取个长边尺度
					self._scale = padded_w / float(self.template_size) #
				else:
					self._scale = padded_h / float(self.template_size)
				self._tmpl_sz[0] = int(padded_w / self._scale) #长边放大到template_size
				self._tmpl_sz[1] = int(padded_h / self._scale) #短边放大scale尺寸
			else:
				self._tmpl_sz[0] = int(padded_w) #否则就使用padding之后的尺寸
				self._tmpl_sz[1] = int(padded_h)
				self._scale = 1.

			if(self._hogfeatures): # True 
				self._tmpl_sz[0] = int(self._tmpl_sz[0]) / (2*self.cell_size) * 2*self.cell_size + 2*self.cell_size # 物体w ÷ 两倍单元格尺寸 x ；这里应该是想获取以cell_size为单位大小的_tmpl_sz吧，然后最外圈再添加一圈cell_size
				self._tmpl_sz[1] = int(self._tmpl_sz[1]) / (2*self.cell_size) * 2*self.cell_size + 2*self.cell_size
			else:
				self._tmpl_sz[0] = int(self._tmpl_sz[0]) / 2 * 2
				self._tmpl_sz[1] = int(self._tmpl_sz[1]) / 2 * 2

		extracted_roi[2] = int(scale_adjust * self._scale * self._tmpl_sz[0]) #w#提取roi
		extracted_roi[3] = int(scale_adjust * self._scale * self._tmpl_sz[1]) #h
		extracted_roi[0] = int(cx - extracted_roi[2]/2) # x
		extracted_roi[1] = int(cy - extracted_roi[3]/2) # y

		z = subwindow(image, extracted_roi, cv2.BORDER_REPLICATE)#裁剪出目标图像
		if(z.shape[1]!=self._tmpl_sz[0] or z.shape[0]!=self._tmpl_sz[1]):#如果裁剪之后图像尺寸变化
			#import pdb;pdb.set_trace()
			z = cv2.resize(z, tuple([int(i) for i in self._tmpl_sz]) ) #则resize到原来大小

		if(self._hogfeatures):
			mapp = {'sizeX':0, 'sizeY':0, 'numFeatures':0, 'map':0}
			mapp = fhog.getFeatureMaps(z, self.cell_size, mapp) #计算hog特征map
			mapp = fhog.normalizeAndTruncate(mapp, 0.2) #叠块直方图归一化
			mapp = fhog.PCAFeatureMaps(mapp) #也不知道是啥了就当收集hog吧
			self.size_patch = [int(i) for i in [mapp['sizeY'], mapp['sizeX'], mapp['numFeatures']] ]#map(int, )
		...
		if(inithann):
			self.createHanningMats()  # createHanningMats need size_patch #创建汉宁窗
	...
	def detect(self, z, x):
		...
		res = real(fftd(complexMultiplication(self._alphaf, fftd(k)), True))#感觉这里是先对特征做傅里叶变换，再与之前的特征 做乘法。在做傅里叶变换
		#这里相当于做卷积，然后匹配最大的位置就是检测中心

		_, pv, _, pi = cv2.minMaxLoc(res)   # pv:float  pi:tuple of int #这个函数是找矩阵中最小值，最大值，最小值索引，最大值索引的
		p = [float(pi[0]), float(pi[1])]   # cv::Point2f, [x,y]  #[float,float] #最大值索引就是中心坐标
		...
		p[0] -= res.shape[1] / 2. #相当于中心比例
		p[1] -= res.shape[0] / 2.

		return p, pv

	def train(self, x, train_interp_factor): #train_interp_factor类似学习率
		k = self.gaussianCorrelation(x, x) #高斯相关
	...
	def init(self, roi, image):
		#import pdb;pdb.set_trace()
		self._roi = [ float(i) for i in roi ]#map(float, roi)
		assert(roi[2]>0 and roi[3]>0)
		self._tmpl = self.getFeatures(image, 1) #获取感兴趣区域hog特征，并加汉宁窗滤波
		self._prob = self.createGaussianPeak(self.size_patch[0], self.size_patch[1]) #没看，应该是创建高斯峰？
